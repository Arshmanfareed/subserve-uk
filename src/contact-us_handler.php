...
$name = filter_var(trim($_POST["firstname"].' '.$_POST["lastname"]), FILTER_SANITIZE_STRING);
...
if(isset($_FILES['fileUpload'])){
      ...
      $expensions= array("txt","csv","png","pdf"."xls");
     
      if(in_array($file_ext,$expensions)=== false){
          $response['status'] = 'error';
        $response['message'] = 'extension not allowed, please choose a .PDF, .txt, .csv, .xls';
      ...
      if(empty($errors)==true) {
         ...
         move_uploaded_file($file_tmp,"../upload_files/".$file_name); //The folder where you would like your file to be saved
        }
   }
...
if (empty($name) || empty($email) || empty($phone) || empty($message)) {
    ...
    try {
        ...
        $mail->clearAddresses();
        $mail->addAddress('sqb330@gmail.com', 'Website Admin');
        if(!empty($file_name))
        {
$mail->addAttachment("../upload_files/".$file_name);
}
        $mail->isHTML(true);
        $mail->Subject = "Contact Form Submission from " . $name;
        $msg_admin = '<p>You have received a new contact form submission:</p>';
        if(!empty($_POST["appropriate"])){
        $msg_admin .= '<H3><strong></strong>Selected Categories</H3>';  
        for($i=0;$i<count($_POST["appropriate"]);$i++){
           $msg_admin .= '<p style="color:blue;"><strong>'.$i.' =>  </strong> <strong>' . $_POST["appropriate"][$i] . '</strong></p>';  
        }
        }
        $msg_admin .= '<p><strong>Name:</strong> ' . $name . '</p>';
        $msg_admin .= '<p><strong>Email:</strong> ' . $email . '</p>';
        $msg_admin .= '<p><strong>Phone:</strong> ' . $phone . '</p>';
        $msg_admin .= '<p><strong>Company name:</strong> ' . $companyname . '</p>';
        $msg_admin .= '<p><strong>Country:</strong> ' . $country . '</p>';
