...
try {
    ...
    if (isset($_GET['id'])) {
        ...
        $stmt = $pdo->prepare("UPDATE blogs SET is_archive = 0 WHERE id = ?");
        if ($stmt->execute([$blogId])) {
            header("Location: all-blogs.php?message=Blog unarchived successfully");
            exit;
        } else {
            echo "Error: Unable to unarchive the blog.";
        }
    } else {
